...
class FreeboxCommander(Cmd):
		...
		for file in  self._fb.get_file_list(self.current_path_name)['result']:
			if file["name"] not in [".", ".."]:
				if args == "-l":
					...
					print file['name']
				self._current_file_list.append(file)
	...
	def complete_cd(self, text, line, begidx, endidx):
		current_directories = self.build_current_files("dir")
		return  [i for i in current_directories if i.lower().startswith(text.lower())]
	...
	def do_get(self, args):
		...
		for file in self._current_file_list:
			if file["name"] == args:
				self._fb.download_file(file['path'], file['name'])


	def complete_get(self, text, line, begidx, endidx):
		current_files = self.build_current_files("file")
		return  [file for file in current_files if file.lower().startswith(text.lower())]
	...
	def build_current_files(self, type):
		current_files = []
		for file in  self._fb.get_file_list(self.current_path_name)['result']:
			if file["type"] == type and file["name"] not in [".", ".."]:
				current_files.append(file['name'].encode('utf-8'))
